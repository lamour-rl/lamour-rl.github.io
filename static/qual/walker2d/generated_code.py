...
def is_recovered(state: np.ndarray) -> int:
    ...
    min_torso_height = 0.8  # Minimum height for torso to be considered upright
    max_torso_angle = np.pi / 6  # Maximum angle deviation from upright position

    # Check if the torso is upright and above the minimum height
    if state[0] > min_torso_height and abs(state[1]) < max_torso_angle:
        return 1
    return 0
...
def calculate_reward(state: np.ndarray, action: np.ndarray) -> float:
    ...
    """
    # Define recovery criteria
    min_torso_height = 0.8
    max_torso_angle = np.pi / 6

    # Calculate distance from recovery criteria
    height_diff = max(0, min_torso_height - state[0])
    angle_diff = max(0, abs(state[1]) - max_torso_angle)
...
# Note: The functions assume that the state and action inputs are valid NumPy arrays
# and do not contain NaN or infinity values.